...
import (
	...
	"log"
	"net/http"
	"creative-service-test/internal/driver"
	"creative-service-test/internal/models"
	"creative-service-test/internal/repository"
	"creative-service-test/internal/repository/dbrepo"
	"os"
	"time"
...
func main() {
	...
	flag.StringVar(&cfg.db.dsn, "dsn", "host=localhost port=5432 dbname=creative-service-test user=postgres password=samiul@10526 sslmode=disable", "DSN")
	...
	pool, err := pgxpool.New(context.Background(), "postgres://postgres:samiul@10526@localhost/creative-service-test")
	if err != nil {
		log.Fatal(err)
	}
	defer pool.Close()
